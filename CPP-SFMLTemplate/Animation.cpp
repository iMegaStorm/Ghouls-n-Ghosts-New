...
void Animation::Update(int row, float deltaTime)
{
	...
	if(totalTime >= switchTime) //if totalTime is bigger or equal to switchTime then do the following
	{
		...
		if(currentImage.x >= imageCount.x) //If the currentImage is bigger than the image count 
		{
			currentImage.x = 0; //Then set it back to zero
		}
	}

	uvRect.left = currentImage.x * uvRect.width; //Setting the offset of the uvRect
